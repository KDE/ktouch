...
*/
    ...
    if (m_keyboardFiles.contains(Prefs::currentKeyboardFile() )==0) {
        ...
        while (it != m_keyboardFiles.constEnd()   &&   (*it).indexOf(fname) == -1) {
            ++it;
        }
...
void KTouch::updateFontFromLecture() {
    ...
    if (!m_lecture.m_fontSuggestions.isEmpty()) {
        ...
        if (f.fromString(m_lecture.m_fontSuggestions)) {
            m_textLineWidget->setFont(f);
        }
        else if (f.fromString("Monospace")) {
            m_textLineWidget->setFont(f);
        }
    }
}
...
void KTouch::updateKeyboardActionCheck() {
    ...
    if (it == m_keyboardFiles.end()) {
        m_keyboardLayoutAction->setCurrentItem(-1);
    }
    else {
        m_keyboardLayoutAction->setCurrentItem(num);
    }
